...
public class ListaCardsPendentesActivity extends AppCompatActivity {


    private RecyclerView mRecyclerView;
    private List<AnuncioB> anunciosPendentes;
    private AnunciosBAdapter adapter;
    private SearchView svAnunciosPendentes;


    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_lista_cards_pendentes);

        mRecyclerView = findViewById(R.id.rvListaPendentesx);
        mRecyclerView.setHasFixedSize(true);
        mRecyclerView.setLayoutManager(new LinearLayoutManager(this));

        svAnunciosPendentes = findViewById(R.id.svPesquisaPendentes);
...
//      add the back button
    ...
    private void buscarAnuncio() {
        ...
        Call<List<AnuncioB>> call = new RetrofitConfig(okHttpClient).getRestInterface().listarAnunciosPendentes();
        call.enqueue(new Callback<List<AnuncioB>>() {
            ...
            public void onResponse(Call<List<AnuncioB>> call, Response<List<AnuncioB>> response) {
                if (response.isSuccessful()) {
                    anunciosPendentes = response.body();

                    if (anunciosPendentes != null) {
                        adapter = new AnunciosBAdapter(anunciosPendentes, getApplicationContext(), ListaCardsPendentesActivity.this);
    ...
    private void buscarSearchView() {
        ...
        svAnunciosPendentes.setOnQueryTextListener(new SearchView.OnQueryTextListener() {
            @Override
